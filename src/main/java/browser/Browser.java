...
public class Browser {
    public static WebDriver createDriver(){
        //объявляем драйвер, но пока что пустой
        WebDriver driver;
        //импортируем переменную из Config
        switch (BROWSER_TYPE){
            case "chrome":
                WebDriverManager.chromedriver().setup();
                ChromeOptions chromeOptions = new ChromeOptions();
                // тесты выполняются, но мы не будем видеть процесс их выполнения
//                chromeOptions.addArguments("--headless");
            ...
            case "firefox":
                ...
                firefoxOptions.addArguments("--healess");
                firefoxOptions.setCapability(CapabilityType.PAGE_LOAD_STRATEGY, "eager");
                driver = new FirefoxDriver(firefoxOptions);
                break;
            // если у нас не эти 2 браузера, то какие действия должны выполниться:
            default:
